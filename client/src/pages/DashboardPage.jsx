...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-obsidian)] flex">
      {/* Sidebar */}
      <aside className="w-72 bg-[var(--color-charcoal)] border-r border-[rgba(201,169,110,0.08)] flex flex-col">
        {/* Logo */}
      ...
      <main className="flex-1 p-8 overflow-auto">
        ...
        >
          <h1 className="text-3xl font-bold text-[var(--color-ivory)] mb-2" style={{ fontFamily: 'var(--font-serif)' }}>
            Bienvenue{user?.name ? `, ${user.name.split(' ')[0]}` : ''} 👋
          </h1>
          <p className="text-[var(--color-white-muted)] mb-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {modules.map((mod, i) => (
              ...
              >
                ...
                <h3 className="text-lg font-bold text-[var(--color-ivory)] mb-1 group-hover:text-[var(--color-champagne)] transition-colors" style={{ fontFamily: 'var(--font-serif)' }}>
                  {mod.label}
                </h3>
                <p className="text-sm text-[var(--color-white-muted)]">{mod.desc}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
